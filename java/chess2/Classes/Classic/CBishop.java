...
public class CBishop implements PieceInf
{
	...
	public CBishop(char team, char moveSet, char contents, boolean hasMoved, int tier)
	{
		this.team = team;
				...
				if ((row + a) >= 0 && (row + a) <= 7 && (col + a) >= 0
					...
					if (((theGame.getTheGame())[row + a][col + a]
						...
						if (((theGame.getTheGame())[row + a][col + a]
								...
								&& 
								(theGame.getTheGame()[row+a][col+a].getContents().getMoveSet() != 'N' || theGame.getTheGame()[row+a][col+a].getContents().getContents() != 'q')) {
