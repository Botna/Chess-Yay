...
public class EBishop implements PieceInf
{
	...
	public EBishop(char team, char moveSet, char contents, boolean hasMoved, int tier)
	{
		this.team = team;
	...
	public GameBoard highlight(GameBoard theGame, int row, int col) {
		...
		PieceInf tempPiece = new CBishop(team, variant, contents, hasMoved,0);
		tempPiece.highlight(theGame, row, col);
		tempPiece = null;
		if(knight)
		{
			tempPiece = new CKnight(team, variant, contents, hasMoved,0);
			tempPiece.highlight(theGame, row, col);
			tempPiece = null;
		}
